...
import { debounceTime, distinctUntilChanged, first, forkJoin, map, merge, Observable, OperatorFunction, Subject, takeUntil, timer } from 'rxjs';
...
export class ExpenseListComponent implements OnDestroy {
    ...
    constructor(
                ...
                this.expenseFilterService.filter$
                    ...
                    .subscribe(filter => {
                        if (filter.categoryId) {
                            const filteredCategory = this.categories.find(c => c.id === filter.categoryId) ?? null;
        ...
        this.expenseService.expenseListUpdate$
            ...
            this.categoryFilterCtrl.valueChanges
                ...
                .subscribe(val => {
                    this.expenseFilterService.updateFilter({ categoryId: val ? val.id : null });
                    this.refreshExpenseList();
                });
    }
    ...
    private refreshExpenseList(): void {
        this.loading = true;
        forkJoin([this.expenseService.getExpensesByMonth(), timer(750)])
            .subscribe(([data]) => {
                this.expenses = data;
                this.loading = false;
            });
